...
packages <- c("stringi","stringr","data.table","parallel")
...
###load street abbreviations look-up table###

#USPS_abbrev <- read.csv("/home/health.dohmh.nycnet/gculp/build_packages/package_data/USPS_abbrev.csv",stringsAsFactors=FALSE)
USPS_abbrev <- read.csv("USPS_abbrev.csv",stringsAsFactors=FALSE)
...
build_rNYCclean_data <- function(pad_version,snd_version,this.dir,my.cores,as_rdb=TRUE) {

	###limits to available cores###
	my.cores <- min(detectCores()-1,my.cores)

	###create temp file##
	temp <- tempfile()
	
	##############################
	###download version of RPAD###
	##############################
	...
	str_pat2 <- build_regex_bank(df_bobaadr,df_snd,my.cores)
	...
	if(as_rdb) {
		...
		pathz <- file.path(this.dir, "Rdata")
		tools:::makeLazyLoadDB(objs, pathz, compress = TRUE)
	} else {
		save(str_pat2, file = file.path(this.dir, "str_pat2.rda"))
		NYC_address_bank <- objs[["NYC_address_bank"]]
		save(NYC_address_bank, file = file.path(this.dir, "NYC_address_bank.rda"))
	}	
...
build_regex_bank <- function(df_bobaadr,df_snd,my.cores) {
	...
	temp_dt[, num.sp:=(str_count(full_stname, "\\s"))+1]
	...
	temp_dt[, orig := trimws(str_replace(full_stname, paste0("^(",strrep("\\S+\\s",word_seq),").*$"), "\\1"))]
	...
	###if only one match is made, pass and remove from pool###
	#yyy_pass <- unique(pL.yyy[rel_id_cnt==1]$rel_id)
	...
	###add patterns that do not alter original value to passes###
	#yyy_pass <- c(yyy_pass,unique(pL.yyy[!(rel_id %in% yyy_fail)]$rel_id))
	...
	###if only one match is made, pass and remove from pool###
	xxx_pass <- unique(pL.xxx[rel_id_cnt==1]$rel_id)
	...
	###add patterns that do not alter original value to passes###
	#xxx_pass <- c(xxx_pass,unique(pL.xxx[!(rel_id %in% xxx_fail)]$rel_id))
