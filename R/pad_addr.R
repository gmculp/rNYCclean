#' The \code{pad_addr} function performs a substring match between a data frame of NYC addresses and NYC Department of City Planning's (DCP) PAD (Property Address Directory) and returns the PAD address, if available.  A user specified variable ensures that the release or version of the PAD and SND files match that of DCP's Geosupport software the analyst will use to geocode the processed addresses.
#'
...
#'     third_col_type, GBAT_name, return_type = "all")
...
#' df1 <- pad_addr(df,"ADDR.pad","ADDR","BORO_CODE","boro_code","16.3")
#'
#' #get PAD address using zip code
#' df2 <- pad_addr(df,"ADDR.pad","ADDR","ZIP_CODE","zip_code","16.3")


pad_addr <- function(in_df, new_addr_col_name, addr_col_name, third_col_name, third_col_type, GBAT_name, return_type = "all"){
	...
	this_rda <- rda_file_check(GBAT_name,"NYC_address_bank") 
	
	if((addr_col_name %in% o_cols) & (third_col_name %in% o_cols) & !(new_addr_col_name %in% o_cols) & (this_rda != "")) {
		...
		out_df <- data.table::rbindlist(lapply(l_v, function(i){
			...
			dt.y <- unique(get(this_rda)[get(this.col) %in% dt.x$temp_col_123, c("NYC_ab.ADDR1","NYC_ab.ADDR1s",this.col), with=FALSE])
		...
		cat("Please check your column names and release/version of Geosupport.")
	}
