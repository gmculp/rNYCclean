#' The \code{splchk_addr} function performs a spell check on a data frame of NYC addresses with a street name dictionary built from NYC Department of City Planning's (DCP) PAD (Property Address Directory) and SND (Street Name Dictionary) files.  A user specified variable ensures that the release or version of the PAD and SND files match that of DCP's Geosupport software the analyst will use to geocode the processed addresses.   
#'
...
#'     third_col_name, third_col_type, GBAT_name)
...
#' df1 <- splchk_addr(df,"ADDR.splchk","ADDR","ZIP_CODE","zip_code","15.4")
#' 

splchk_addr <- function(in_df, new_addr_col_name, addr_col_name, third_col_name, third_col_type, GBAT_name){
	...
	this_rda1 <- rda_file_check(GBAT_name,"zipcode_words_vector")
	this_rda2 <- rda_file_check(GBAT_name,"borocode_words_vector")

	if(this_rda1 != "" & this_rda2 != ""){
		###more efficient if we remove duplicates###
		temp_df <- unique(in_df[,c(addr_col_name,third_col_name),with=FALSE])
		
		#temp_df[[new_addr_col_name]] <- vector_splchk(temp_df[[addr_col_name]],temp_df[[third_col_name]],third_col_type,GBAT_name)
		temp_df[,temp.name := vector_splchk(get(addr_col_name),get(third_col_name),third_col_type,GBAT_name)]
		...
		cat("Please check your release/version of Geosupport.")
	}
