...
vector_splchk <- function(addr_col,third_col,third_col_type,GBAT_name){

	###determine third column type for look-up table###
	if(grepl("zip",third_col_type,ignore.case=TRUE)){
		zipcode_words_vector <- get(rda_file_check(GBAT_name,"zipcode_words_vector"))
		sorted_words <- split(unname(zipcode_words_vector),names(zipcode_words_vector))
	} else{
		borocode_words_vector <- get(rda_file_check(GBAT_name,"borocode_words_vector"))
