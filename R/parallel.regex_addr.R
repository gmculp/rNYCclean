...
parallel.regex_addr <- function(in_clus, in_df, new_addr_col_name, addr1_col_name, addr2_col_name=NULL, boro_col_name=NULL){
	...
	if(!(missing(addr1_col_name))){
		...
		if (in_clus <= 1) {
			...
			out_df <- regex_addr(in_df, new_addr_col_name, addr1_col_name, addr2_col_name, boro_col_name)
			...
			if(.Platform$OS.type != "windows"){
				...
				regex_addr(in_df[pc==zz,], new_addr_col_name, addr1_col_name, addr2_col_name, boro_col_name),mc.cores=in_clus))
				...
				parallel::clusterExport(clus, c("in_clus","in_df","new_addr_col_name",'addr1_col_name','addr2_col_name','boro_col_name'), envir=environment())
				
				out_df <- data.table::rbindlist(parallel::parLapply(clus, 1:in_clus, function(zz) 
				regex_addr(in_df[pc==zz,], new_addr_col_name, addr1_col_name, addr2_col_name, boro_col_name)))
