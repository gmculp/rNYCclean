...
#' @param boro_col_name the name of the borough code column as string.  Optional.
...
#'     addr1_col_name, GBAT_name, addr2_col_name = NULL, 
#'     boro_col_name = NULL)
...
#' df1 <- parallel.regex_addr(in_clus=2, in_df = df, 
#'     new_addr_col_name = "ADDR.regex", addr1_col_name = "ADDR1", 
#'     boro_col_name = "BORO_CODE")
#' 
#' #two address input column without borough code
#' df2 <- parallel.regex_addr(in_clus=2, in_df = df, 
#'     new_addr_col_name = "ADDR.regex", addr1_col_name = "ADDR1", 
#'     addr2_col_name = "ADDR2")

parallel.regex_addr <- function(in_clus, in_df, new_addr_col_name, addr1_col_name, addr2_col_name=NULL, boro_col_name=NULL){
	...
	if(!(missing(addr1_col_name))){
		...
		if (in_clus <= 1) {
			...
			out_df <- regex_addr(in_df, new_addr_col_name, addr1_col_name, GBAT_name, addr2_col_name, boro_col_name)
			...
			if(.Platform$OS.type != "windows"){
				...
				regex_addr(in_df[pc==zz,], new_addr_col_name, addr1_col_name, GBAT_name, addr2_col_name, boro_col_name),mc.cores=in_clus))
				...
				parallel::clusterExport(clus, c("in_clus","in_df","new_addr_col_name",'addr1_col_name','addr2_col_name','boro_col_name','GBAT_name'), envir=environment())
				
				out_df <- data.table::rbindlist(parallel::parLapply(clus, 1:in_clus, function(zz) 
				regex_addr(in_df[pc==zz,], new_addr_col_name, addr1_col_name, GBAT_name, addr2_col_name, boro_col_name)))
		...
		cat("Please check you column names and your release/version of Geosupport!!!")
	}
}	