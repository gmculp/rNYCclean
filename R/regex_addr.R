...
regex_addr <- function(in_df, new_addr_col_name, addr1_col_name, addr2_col_name=NULL, boro_col_name=NULL){
	...
	if(!is.null(boro_col_name)) {
		u_df <- unique(in_df[,c("temp_addr_col",boro_col_name),with=FALSE])
		u_addr <- u_df$temp_addr_col
		u_boro <- as.character(u_df[,get(boro_col_name)])
	} else {
	...
	if(!(exists("str_pat2"))) cat("Package data loading...\n")	
	
	#run through one-of-a-kind street names
	...
	out_df[,new_addr_col_name := ifelse(num1=="","",str_replace(b, paste0(".*?(",num1,")"), "\\1"))]
